...
$sql1 = "SELECT * FROM mt_leave WHERE  LEAVE_ID = '$leave_id'  AND   L_TYPE = 'casual leave' AND EMP_NO IN (SELECT EMP_NO FROM mt_emp WHERE dept_id=8) ";
...
$sql2 = "SELECT * FROM  coff  WHERE  LEAVE_ID = '$leave_id'  ";
...
{
    $remark = $_POST['hod_remark'];
    $query = "UPDATE mt_leave SET L_TYPE='lwp',HOD_APPROVED = 'Rejected',HOD_REMARKS='$remark' WHERE LEAVE_ID = '$leave_id' ";
    $res = mysqli_query($conn , $query);
    if($res)
    {
		            require 'vendor/autoload.php'; 
				...
				<div class="forms" >
					<div class="form-grids row widget-shadow" data-example-id="basic-forms"> 	
							...
							<form class="form-horizontal" method="POST" action="#">
								<input type="hidden" name="leave_type" value="compensatory off">
								
							 	<h2 style="text-align: center; margin-top: 15px;">APPLICATION FOR COMPENSATORY LEAVE FOR<?php echo "$department" ?></h2><br>
								...
								<div>	<label  for="HOD_APPROVED" class="col-sm-3 control-label">HOD APPROVED REMARKS :</label>
									<div class="col-sm-6">
										<input type="text" class="form-control"  name="hod_remark" name="HOD_APPROVED" style="margin-top: -30px;" > 									</div>	
								</div>
								<div class="col-sm-2" >
